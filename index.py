...
def process(data):
    ...
    df.loc[-1] = ['страховой номер', 'фамилия', 'собственное имя', 'отчество (если таковое имеется)',
                  'дата приема на работу', 'дата приказа о приеме на работу', 'номер приказа о приеме на работу',
                  'дата увольнения с работы', 'дата приказа об увольнении с работы', 'номер приказа об увольнении с работы', 'код основания увольнения',
                  'код должности служащего, профессии рабочего', 'наименование должности служащего, профессии рабочего', 'наименование структурного подразделения',
                  'код работы по совместительству',
                  'дата приема', 'дата приказа о назначении на должность служащего (профессию рабочего)', 'номер приказа о назначении на должность служащего (профессию рабочего)', 'код вида трудового договора',
                  'дата увольнения', 'дата приказа об увольнении с работы', 'номер приказа об увольнении с работы', 'код основания увольнения',
                  'дата присвоения квалификационной категории', 'дата приказа о присвоении квалификационной категории', 'номер приказа о присвоении квалификационной категории',
                  'разряд', 'квалификационная категория', 'класс', 'класс государственного служащего']
    df.index = df.index + 1
    df = df.sort_index()
